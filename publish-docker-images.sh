...
function check_amazon_ecr () {
    local VERSIONARCH="$1"
    echo "running 'aws --region ${ECR_AWS_REGION:-eu-west-1} ecr describe-images --registry-id ${ECR_AWS_ACCOUNT_ID} --repository=${ECR_REPOSITORY_NAME:-elastic/elasticsearch} --image-ids=imageTag=${VERSIONARCH} 1>/dev/null 2>&1'.." >&4
    if aws --region ${ECR_AWS_REGION:-eu-west-1} ecr describe-images --registry-id ${ECR_AWS_ACCOUNT_ID} --repository=${ECR_REPOSITORY_NAME:-elastic/elasticsearch} --image-ids=imageTag=${VERSIONARCH} 1>/dev/null 2>&1 ; then
        return 1
    else
        return 0
    fi
    ...
    if is_dryrun_mode ; then
        cat - | while read VERSIONARCH ; do
            if is_custom_base_url_overridden ; then
                ...
                echo "running 'aws --region ${ECR_AWS_REGION:-eu-west-1} ecr describe-images --registry-id ${ECR_AWS_ACCOUNT_ID} --repository=${ECR_REPOSITORY_NAME:-elastic/elasticsearch} --image-ids=imageTag=${VERSIONARCH} | jq -r '.''.." >&4
                aws --region ${ECR_AWS_REGION:-eu-west-1} ecr describe-images --registry-id ${ECR_AWS_ACCOUNT_ID} --repository=${ECR_REPOSITORY_NAME:-elastic/elasticsearch} --image-ids=imageTag=${VERSIONARCH} | jq -r '.'
                echo "running 'aws --region ${ECR_AWS_REGION:-eu-west-1} ecr describe-images --registry-id ${ECR_AWS_ACCOUNT_ID} --repository=${ECR_REPOSITORY_NAME:-elastic/elasticsearch} | jq -r '.''.." >&4
                aws --region ${ECR_AWS_REGION:-eu-west-1} ecr describe-images --registry-id ${ECR_AWS_ACCOUNT_ID} --repository=${ECR_REPOSITORY_NAME:-elastic/elasticsearch} | jq -r '.'
            else
                if dryrun_enable_real_check ; then
